      ...
      >
        ...
        <div className="flex items-center justify-between p-8 pb-0">
          <h2 className="text-lg font-bold text-white">
            {loading ? "Criando link..." : createdUrl ? "Link criado!" : "Criar Novo Link"}
          </h2>
          {!loading && (
            <button
              onClick={onClose}
        ...
        {/* Loading state */}
        {loading ? (
          <div className="px-8 pb-8 flex flex-col items-center gap-5 py-6">
            <svg className="animate-spin w-8 h-8" viewBox="0 0 24 24" fill="none" style={{ color: "#ffe033" }}>
              <circle className="opacity-20" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
              <path className="opacity-90" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
            <p className="text-sm font-medium" style={{ color: "rgba(255,255,255,0.5)" }}>
              Criando e gerando seu link...
            </p>
          </div>
        ) : createdUrl ? (
          /* Success state */
